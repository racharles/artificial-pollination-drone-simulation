...
MonoNode::MonoNode (ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) : Node (sensor, node_handle, image_transport) {
  image_subscriber = image_transport.subscribe ("/camera/rgb/image_raw", 1, &MonoNode::ImageCallback, this); // /camera/image_raw  /usb_cam/image_raw
  camera_info_topic_ = "/camera/rgb/camera_info"; // /camera/camera_info   /usb_cam/camera_info
}
