...
namespace gazebo
{
  class FruitGenerator : public WorldPlugin
  {
    public: void Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf)
    {
        ...
        home.append("/.gazebo/models/tomato_plant/meshes/plant.dae"); // Assume all trees are oak trees.
        ...
        for(auto m : models){
            ...
            if(model_name.find("plant")!=std::string::npos)//It is a tree then
        ...
        apple_path.append("/.gazebo/models/plastic_cup/meshes/plastic_cup.dae");
        ...
        for(int i=0;i<mesh->GetVertexCount()*3;i+=3){
            ...
            if(vertices[i+2]>0.5 && vertices[i+2]<4.5 && planar_distance < 2){//Only consider the vertices higher than 2 meters and lower than 4.5
